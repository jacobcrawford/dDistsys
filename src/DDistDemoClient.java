...
public class DDistDemoClient {
    ...
    protected Thread listenForServerInput;
	...
	if (socket != null) {
		...
		while ((s = stdin.readLine()) != null && !toServer.checkError()) {
		    toServer.println("From client:" + s);
		}
		socket.close();
	    } catch (IOException e) {
	...
	private void startListeningForInput(final Socket socket) {
		listenForServerInput = new Thread(new Runnable() {
			@Override
			public void run() {
